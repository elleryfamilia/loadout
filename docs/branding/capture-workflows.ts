...
// Navigate to the workflows view (Library tab -> workflows segment).
for (const target of ["/tab/library"]) {
  const click = await send("Runtime.evaluate", {
    expression: `
      (() => {
        const el = [...document.querySelectorAll('[hx-get]')].find(e => e.getAttribute('hx-get') === '${target}');
  ...
  const v = click.result?.result?.value;
  console.log("click:", JSON.stringify(v));
  if (v && v.startsWith("clicked")) break;
}
await new Promise((r) => setTimeout(r, 1200));
const wf = await send("Runtime.evaluate", {
  expression: `
    (() => {
      const el = [...document.querySelectorAll('[hx-get]')].find(e => e.getAttribute('hx-get') === '/library/workflows');
      if (!el) return "no workflows segment";
      el.click();
      return "clicked /library/workflows";
    })()`,
  returnByValue: true,
});
console.log("wf:", JSON.stringify(wf.result?.result?.value));
await new Promise((r) => setTimeout(r, 1500)); // htmx swap
const attrs = await send("Runtime.evaluate", {
  expression: `JSON.stringify([...document.querySelectorAll('[hx-get]')].map(e => e.getAttribute('hx-get')).slice(0, 40))`,
  returnByValue: true,
});
console.log("hx-gets now:", attrs.result?.result?.value);
