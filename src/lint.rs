...
///
/// **Adding a pattern (binding):** any new *text* pattern should be
/// case-insensitive (`(?i)`, as every text pattern here is), so the same text
/// cannot produce different verdicts depending on how it was cased or
/// whitespace-collapsed upstream. A character-class pattern (the hidden-Unicode
/// rules below) is inherently invariant and needs no `(?i)`.
