...
pub fn run(rt: &Runtime, args: &HookArgs) -> crate::Result<()> {
    ...
    // nothing at all.
    //
    // Deliberately the FIRST thing `run` does: before config load, repo
    // discovery, and agent resolution. Any of those can fail (an unparseable
    // config, no `$HOME`, an agent id this binary no longer knows), and a stale
    // hook that exits non-zero is exactly the breakage this exists to prevent.
    //
    // Keep this indefinitely. It is a handful of lines, and a machine can
    // upgrade 0.18 → 0.21 without ever running the version that cleans up.
...
fn remove(hr: &HookRegistry, dry_run: bool) -> crate::Result<()> {
    ...
    // `None`: `--remove` is the user asking for every loadout entry in this file
    // to go, whatever event it sits on. The event scope exists for the automatic
    // learning cleanup, which must touch only what it registered.
    match adapters::remove_hook_command(&existing, &hr.subcommand, None)? {
        Some(updated) => {
            if dry_run {
                println!(
