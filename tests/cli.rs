...
/// VS Code reads `.github/instructions/*.instructions.md` as a default-on
/// location and does NOT gitignore-filter it; the frontmatter (`applyTo: '**'`)
/// must be the file's FIRST bytes, before the generated-marker header.
#[test]
fn copilot_writes_gitignored_vscode_target_with_frontmatter_first() {
    let fx = Fixture::new();
    fx.rust_project();
    fx.rust_profile();
    fx.git_init();
    ...
    assert!(
        target.starts_with("---\n"),
        "frontmatter must be the first bytes; got:\n{}",
        &target[..target.len().min(200)]
    );
    let fm_end = target[3..].find("---").expect("closing frontmatter fence") + 3;
    assert!(target[..fm_end].contains("applyTo: '**'"));
    // The generated-marker header follows the frontmatter (freshness detection).
    assert!(target.contains("<!-- loadout:generated context=sha256:"));
    assert!(target.find("---").unwrap() < target.find("loadout:generated").unwrap());
    // Guidance body made it in — full content, not an @import pointer.
    assert!(target.contains("Rust project. Build with cargo"));
    // Wired file is gitignored.
    assert!(fx
        .read(".gitignore")
        .contains(".github/instructions/loadout.instructions.md"));
}

/// Re-rendering with an unchanged context must not churn the target file.
#[test]
fn copilot_rerender_is_idempotent() {
    let fx = Fixture::new();
    fx.rust_project();
    fx.rust_profile();
    ...
    fx.cmd()
        ...
        .assert()
        .success();
    let first = fx.read(".github/instructions/loadout.instructions.md");
    ...
    assert_eq!(
        first,
        fx.read(".github/instructions/loadout.instructions.md"),
        "unchanged context must not rewrite (timestamp would churn)"
    );
}

/// A pre-existing instructions file the user wrote themselves is never
/// overwritten — loadout warns and leaves it alone.
#[test]
fn copilot_refuses_to_overwrite_foreign_instructions_file() {
    let fx = Fixture::new();
    fx.rust_project();
    fx.rust_profile();
    fx.write(
